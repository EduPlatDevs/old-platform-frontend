...
export const LoginPage = ({ ...others }) => {
  ...
  const handleGoogle = async (response) => {
    ...
    dispatch(SET_LOADING, true);
    await userGoogleLogin(response)
      .then(async (result) => {
        if (result.status === 'error') {
          if (result.message === "Cannot read property 'email' of null") {
            setErrorMsg('This user does not exist. Register first');
        ...
        <Box
          ...
          <Grid
            ...
            <Grid
              ...
              <Typography variant='body2'>
                {i18next.t('IfyouhaveaGoogleGmailaccount')}{' '}
                {i18next.t('loginhere')}
              </Typography>
            </Grid>
            <Grid
          ...
          <Formik
            ...
            validationSchema={Yup.object().shape({
              email: Yup.string()
                .email(i18next.t('Mustbeavalidemail'))
                .max(255)
                .required(i18next.t('Emailisrequired')),
              password: Yup.string()
                .max(255)
