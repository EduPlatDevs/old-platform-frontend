...
export const ResourcesNetflixGrid = (props) => {
  ...
  const dangerouslyProceedToDelete = async (event) => {
    try {
      if (eduToDelete) {
        await deleteResource(eduToDelete)
          .then((data) => {
            console.log(data);
            setDeleteDialog(false);
          })
  ...
  return (
    <>
      <Grid
        container
        sx={{ backgroundColor: newcolor, mt: mt }}>
        <Grid
          container
          direction='row'
          ...
          {defaultMode && defaultMode === 'List' ? (
            ...
            <>
              <Grid item>
                <ButtonGroup>
                  <IconButton
                    color={mode === 'Grid' ? 'primary' : ''}
                    onClick={HandelGridClick}>
                    <GridOnIcon />
                  </IconButton>
                  <IconButton
                    color={mode === 'List' ? 'primary' : ''}
                      ...
                      {defaultMode && defaultMode === 'List' ? (
                        <>
                          <ButtonGroup>
                            <Button
                              onClick={(e) => {
                                editClickHandle(e, edusource);
                              }}
                              size='small'
                              sx={{ borderRadius: '15px', mt: 1 }}>
                              {i18next.t('edit')}
                            </Button>
                            <Button
                              onClick={(e) => {
                                deleteCliclHandle(e, edusource);
                              }}
                              size='small'
                              sx={{ borderRadius: '15px', mt: 1 }}>
                              {i18next.t('delete')}
      ...
      <Dialog
        ...
        <DialogActions>
          ...
          <Button onClick={dangerouslyProceedToDelete}>
            {' '}
            {i18next.t('Agree')}{' '}
          </Button>
        </DialogActions>
